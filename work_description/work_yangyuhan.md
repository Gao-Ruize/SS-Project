...
- 开始筹备将JWTUtils与实际后端接口测试结合在一起
- 初试JWT与Filter结合,写了一个测试demo,从 请求 的header 从获取传入的 token 并进行校验，校验通过则请求走下去；否则，抛出异常，在异常统一处理类中处理。

# 2020/7/24

- 与队友分别开发了两套方案:一套是用annotation作token注解,再使用拦截器拦截请求;一套是使用filter,从请求的header中获取token并检验(在这套方案中,我们拟将用户的role写入token中进行检验)
- 归总了一个python脚本,用于:爬数据—检验数据—判断是否需要发送消息(同时储存到数据库)—更新数据,以装载服务器时能够有效设置定时运行某一个脚本
...
- 为python脚本编写了日志(错误处理/成功处理 当遇见Exception时/操作成功时都会储存入日志中)
- 学习了一下容器以及脚本挂载服务器的相关知识
- 配置了后端性能调优的环境

# 2020/7/28

- 完成了JMeter, Docker等安装以及环境的调试
- 学习了Docker的相关知识,操作等
- 完成了Grafana+Prometheus的springboot监控平台的搭建,并初步进行了部分压力测试以观察自己搭建的成功与否-
- 考虑原有数据脚本的数据量不足,故写了一个新的数据脚本进行数据填充,以确保能够达到至少10k的数据量

# 2020/7/29

- 昨日的测试脚本出现了部分问题,正在完善中
- 已更改好测试脚本,于jwc_message表中插入了2w条数据—一半是学生接收的一半是老师接收的
- 对于ins_message表,我的插入数据的构思是—拟以50位老师对500位学生 然后每个老师—学生一一对应关系之间插入20条消息,以达到10k的数据量要求
- 用jmeter开始压力测试,发现我们需要首先发登录请求获取token,故学习了一下jmeter的相关知识
- 考虑到后端的Auth等特性,故对脚本完成了一定的修改,先生成学生—老师元组对,再去做相关信息的插入
- 利用jmeter跑了一个比较基础的压力测试,该接口用于查看所有的教务处信息,由数据显示,在30并发的情况下,平均响应时间已经到达了2s,因此确实需要进行性能优化(目前IO方面的消耗是比较大的).

# 2020/7/30

- 进行了初步优化调试,即在原本项目的基础上加上了cache,以观察其优化后的效率
- 可以通过观察得到,在启动类增加了@EnableCaching,以及在方法加了@Cacheable注解后,吞吐量得到了很大程度的提升,并且响应的平均时间也大大的缩减了(虽然在某一瞬间 Java虚拟机的CPU占用率也到达了几乎100%?)
- 上网查阅资料得知,在springboot的项目中,调优主要还是通过配置文件以及配置JVM的参数进行;因此阅读了Springboot以及JVM的部分官方文档
- 与队友沟通交流了解到,主要还是需要对高并发进行调优,因此去了解了一下springboot关于高并发调优的知识
- 没有什么优化的思路,先研究一下脚本挂载aws云
