...
# day19 7.30
- 在使用grafana对压力测试进行分析后，发现进程所占用资源很少，而cpu几乎跑满，分析是由于数据库的压力过大导致相应速度变慢，学习了相关解决措施后在后端添加了cache功能，在本地测试中明显降低了延迟  
- 在完整功能检查中发现后端存在一些问题，修复了bug  
- 暂时使用个人appId申请了模版并用该模版进行服务号通知的发送（现在通知只能发到我自己的微信上，无法获取他人openId）    
...
小程序在绑定后会出现部分数据获取不到的问题，通过调用登陆接口解决  
部分页面会被底部导航栏挡住，添加了占位行
